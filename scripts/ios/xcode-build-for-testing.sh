...
if [[ $build_exit_code -ne 0 ]]; then
  echo "Build for testing failed in ${elapsed}s"
  exit $build_exit_code
fi

echo "Build for testing succeeded in ${elapsed}s"

xctestrun=$(find "$DERIVED_DATA_PATH/Build/Products" -name "*.xctestrun" 2>/dev/null | head -1)
if [[ -n "$xctestrun" ]]; then
